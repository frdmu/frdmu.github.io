...
- 2.根据Figure 4，结点刚开始的状态是Follower，再根据Figure 2的提示，于是初始化结点：
...
- 3.根据Figure 4，Follower会在election timeout之后转换为Candidate，这件事在ticker()中完成：
...
- 4.结点成为Candidate后会发起选举。startElection()的基本逻辑很简单：向其他结点发出投票请求RPCs，再根据回应进行状态转换。根据Figure 4，Candidate此时有两种转换可能，一种是得到大多数支持而转换为Leader，另一种是更新currentTerm转换为Follower。
...
- 5.选举完成后，Leader会定期发出heartbeat，根据Figure 4，Leader的状态转换只有一种可能，就是因为更新currentTerm转换为Follower，具体的实现主要有：周期性向其他结点发出heartbeat RPCs，并根据回应进行状态转换。
